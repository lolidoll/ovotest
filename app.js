...
6. 可以分多条消息回复（重要）`);
        ...
        function extractMindStateFromText(text) {
            ...
            for (const [key, label] of Object.entries(fields)) {
                // 创建匹配模式：支持中文冒号和英文冒号
                const pattern = new RegExp(`${label}[：:]{1,2}\\s*([^【\\n]*?)(?=\\n|[${Object.values(fields).join('')}]|$)`, 'i');
                ...
                if (match && match[1]) {
                    ...
                    // 特殊处理数值字段
                    if (key === 'affinity' || key === 'affinityChange') {
                        // 尝试提取数字
                        const numberMatch = value.match(/(-?\d+)/);
                        mindState[key] = numberMatch ? parseInt(numberMatch[1]) : null;
                    } else {
                        mindState[key] = value || null;
                    }
                }
            }
