        ...
        const AppState = {
            ...
            apiSettings: {
                ...
                // 模型超时配置（毫秒），不同的模型有不同的处理时间
                modelTimeouts: {
                    'gpt-4': 120000,      // GPT-4: 120秒
                    'gpt-3.5-turbo': 60000, // GPT-3.5: 60秒
                    'claude': 180000,     // Claude: 180秒（更复杂的推理）
                    'default': 60000      // 默认: 60秒
                },
                defaultTimeout: 60000 // 默认超时时间
            },
            user: {
                name: '薯片机用户',
                avatar: '', // 侧边栏头像
    ...
    try {
        ...
        async function makeAPIRequest(options) {
            ...
            const getTimeout = () => {
                ...
                // 根据模型名称获取超时时间
                for (const [modelKey, modelTimeout] of Object.entries(AppState.apiSettings.modelTimeouts)) {
                    if (model && model.toLowerCase().includes(modelKey)) {
                        return modelTimeout;
                    }
                }
                return AppState.apiSettings.defaultTimeout;
            };
            ...
            try {
                ...
                clearTimeout(timeoutId);
                const errorInfo = classifyError(error, timeoutMs);
                
                // 重试逻辑
                if (retries > 0 && errorInfo.isRetryable) {
                    console.warn(`API请求失败，将在2秒后重试（剩余次数: ${retries})`, errorInfo);
                    await new Promise(r => setTimeout(r, 2000));
                    return makeAPIRequest({ ...options, retries: retries - 1 });
                }
        ...
        function classifyError(error, timeoutMs) {
            ...
            let isRetryableNow = false;

            if (error.name === 'AbortError') {
                errorType = 'timeout';
                userMessage = `请求超时（${timeoutMs / 1000}秒）`;
                suggestion = '1. 检查网络连接速度\n2. 切换更快速的API端点\n3. 在API设置中增加超时时间\n4. 对于复杂问题，尝试简化输入内容';
                isRetryable = true;
                isRetryableNow = true;

            } else if (error instanceof TypeError && error.message.includes('Failed to fetch')) {
                errorType = 'network';
                userMessage = '网络错误或CORS问题';
                suggestion = '1. 检查网络连接\n2. 确认API端点URL正确\n3. 检查API服务器是否可访问\n4. 查看浏览器控制台了解更多错误信息';
                isRetryable = true;
                isRetryableNow = true;
                ...
                errorType = 'rateLimit';
                userMessage = 'API请求过于频繁（速率限制）';
                suggestion = '1. 等待一段时间后重试\n2. 检查是否多个请求同时发送\n3. 升级API计划以获得更高的速率限制\n4. 实现请求队列，控制并发数量';
                isRetryable = true;
                isRetryableNow = false; // 需要等待

            } else if (error.status === 500 || error.status === 502 || error.status === 503) {
                errorType = 'serverError';
                userMessage = `服务器错误 (${error.status})`;
                suggestion = '1. API服务器目前可能有问题，请稍后重试\n2. 查看服务商的状态页面\n3. 尝试使用备用API端点';
                isRetryable = true;
                isRetryableNow = false; // 稍后重试

            } else if (error.status >= 400) {
                errorType = 'httpError';
                userMessage = `HTTP错误: ${error.status}`;
                suggestion = `检查请求参数是否正确。错误详情: ${error.message}`;
                isRetryable = false;

            } else if (error.message?.includes('model')) {
                errorType = 'modelNotFound';
                userMessage = '指定的模型不存在或无法访问';
                suggestion = '请在API设置中检查：\n1. 模型名称是否正确\n2. 是否有权限使用该模型\n3. 模型是否已启用';
                isRetryable = false;

            } else {
                errorType = 'generic';
                userMessage = error.message || '未知错误';
                suggestion = '请查看浏览器控制台(F12)了解详细错误信息';
            }
        ...
        function translateToChineseViaAPI(text, msg) {
            ...
            // 选择使用哪个API
            const endpoint = hasSecondaryApi ? AppState.apiSettings.secondaryEndpoint : AppState.apiSettings.endpoint;
            const apiKey = hasSecondaryApi ? AppState.apiSettings.secondaryApiKey : AppState.apiSettings.apiKey;
            const model = hasSecondaryApi ? AppState.apiSettings.secondarySelectedModel : AppState.apiSettings.selectedModel;
            ...
            makeAPIRequest({
                ...
                onError: (errorInfo) => {
                    ...
                    if (errorInfo.suggestion) {
                        console.log('错误处理建议:', errorInfo.suggestion);
                    }
                }
            }).catch(error => {
                // 这里会捕获所有无法重试的错误
        ...
        async function callApiWithConversation() {
            ...
            // 为当前会话生成新的API调用回合ID
            apiCallRounds[convId] = 'round_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
            ...
            try {
                ...
                if (systemMsg) {
                    console.log('【系统提示内容】:', systemMsg.content.substring(0, 500));
                }

                const controller = new AbortController();
                // 根据模型选择合适的超时时间
                const modelTimeouts = api.modelTimeouts || {};
                let timeout = modelTimeouts[api.selectedModel] || modelTimeouts['default'] || 60000;
                timeoutId = setTimeout(() => controller.abort(), timeout);
                fetchOptions.signal = controller.signal;
                
                console.log(`⏱️ 使用超时配置: ${timeout}ms，模型: ${api.selectedModel}`);
                    ...
                    if (!res.ok) {
                        ...
                        if (data) {
                            ...
                            if (data.choices && Array.isArray(data.choices) && data.choices[0]) {
                                ...
                                // Anthropic/Claude 格式：choices[0].text
                                else if (choice.text) {
                            ...
                            // 2. Google Gemini 格式：data.candidates[0].content.parts[0].text
                            ...
                            if (assistantText && assistantText.trim()) {
                                ...
                                console.log('【调试API响应】文本长度:', assistantText.length);
                                console.log('【调试API响应】换行符个数:', (assistantText.match(/\n/g) || []).length);
                                console.log('【调试API响应】包含\n\n:', assistantText.includes('\n\n'));
                                console.log('【调试API响应】前300字:', assistantText.substring(0, 300));
                ...
                if (err.name === 'AbortError') {
                    const modelTimeouts = api.modelTimeouts || {};
                    const timeout = (modelTimeouts[api.selectedModel] || modelTimeouts['default'] || 60000) / 1000;
                    lastError = `API 请求超时（${timeout}秒）`;
                    console.error(`请求超时 (${timeout}s):`, endpoint);
...
"我不太同意...【表情包】困惑【/表情包】"`;
        ...
        function collectConversationForApi(convId) {
            ...
            // 获取角色当前的好感度（如果没有历史记录则使用初始值50）
            let currentAffection = 50; // 默认初始值
            if (conv.affectionHistory && conv.affectionHistory.length > 0) {
                currentAffection = conv.affectionHistory[conv.affectionHistory.length - 1].value;
            }
            
            systemPrompts.push(`【心声信息格式】你必须在每次回复结束后，添加一行心声信息。

格式：【心声】好感度：数字|+/-数字|原因 穿搭：内容 心情：内容 动作：内容 心声：内容 坏心思：内容

示例：
【心声】好感度：52|+2|这个话题很有趣 穿搭：上身穿着一件淡蓝色的棉麻衬衫，袖口微微卷起；下装是深灰色的休闲九分裤，脚踩一双白色帆布鞋。左手腕系着一条编织红绳，胸前挂着一枚小小的银杏叶胸针。整体风格干净简约，带着几分慵懒随性。 心情：平静中带着一丝雀跃，像是阴天里透过云层洒下的微弱阳光。上午的事情顺利完成，下午还有期待已久的独处时间。内心有些小满足，但表面上依然维持着淡漠从容的样子。 动作：靠在窗边的懒人沙发上，手指无意识地轻轻敲击着扶手。偶尔抬头望向窗外，似乎在思考什么，又像只是单纯地发呆。翻开一半的书放在手边，茶杯里的水已经凉透了。 心声：今天的阳光真好，要是能一直这样就好了。那件事要不要找个机会说出口呢？其实……有点在意他今天说的那句话。 坏心思：计划偷偷把冰箱里的蛋糕吃掉，然后嫁祸给那只经常来窗台的流浪猫。打算在朋友面前装作若无其事，其实早就猜到了他要说的惊喜是什么。如果明天有人问起，就说自己一整天都在看书，什么都没做。

重要：这一行会被系统自动处理，用户看不到。必须每次都加，不能省略，也不能重复。`);
...
6. 可以分多条消息回复（重要）`);
        ...
        function cleanAIResponse(text) {
            ...
            // 第零层：移除思考过程标记（明确的系统标记）
            // 只移除明确的thinking/reasoning标记，不触及用户内容
            text = text.replace(/<think>[\s\S]*?<\/think>/gi, '');
            text = text.replace(/<thinking>[\s\S]*?<\/thinking>/gi, '');
            text = text.replace(/<thought>[\s\S]*?<\/thought>/gi, '');
            text = text.replace(/<thoughts>[\s\S]*?<\/thoughts>/gi, '');
            text = text.replace(/<internal>[\s\S]*?<\/internal>/gi, '');
            text = text.replace(/<reasoning>[\s\S]*?<\/reasoning>/gi, '');
            text = text.replace(/\{\s*"thought":[\s\S]*?\}/gi, '');
            ...
            // 清理消息格式标签（这些是系统的，不会是用户内容）
            text = text.replace(/\[REPLY\d+\]|\[\/REPLY\d+\]/g, '');
            text = text.replace(/\[MSG\d+\]|\[\/MSG\d+\]/g, '');
            text = text.replace(/\[WAIT(?::[\d.]+)?\]/g, '');
            
            // 第一层：只移除明确的系统标记【心声】【思维链】等（改进逻辑避免误删）
            // 只移除特定的系统标记，不用过于宽泛的正则
            const systemTags = ['心声', '思维链', '思考', '系统', '指令', '提示', '缓冲', '内部', '调试', '日志'];
            for (const tag of systemTags) {
                // 更精确的正则：只匹配【系统标签】及其内容，直到下一个【或行尾
            ...
            text = text.replace(/\[THINKING\][\s\S]*?\[\/THINKING\]/gi, '');
            text = text.replace(/\[THOUGHT\][\s\S]*?\[\/THOUGHT\]/gi, '');
            text = text.replace(/\[mindstate\][\s\S]*?\[\/mindstate\]/gi, '');
            text = text.replace(/\[internal\][\s\S]*?\[\/internal\]/gi, '');
            text = text.replace(/\[debug\][\s\S]*?\[\/debug\]/gi, '');
            
            // 移除JSON格式的思考过程
            text = text.replace(/\{\s*"(thinking|thought|mindstate)"[\s\S]*?\}/gi, '');
        ...
        // API调用回合ID存储（按会话ID作为键，避免多会话冲突）
        let apiCallRounds = {};

        // 最务实的文本分割器：智能拆分AI回复以实现分段推送
        // 参考方案：规则化递进，优先使用结构化分隔符，退而求其次
        function goodEnoughSplitter(text) {
            console.log('【分割器】输入文本长度:', text.length);
            
            // 规则1：太短不分（避免过度拆分）
            if (!text || text.length < 80) {
                console.log('【分割器】规则1：文本太短，不分割');
                return [text];
            }
            
            // **改进：规则2a - 先检测是否有明显的段落分隔（多个换行）**
            // 这比后面的单换行检测更优先，因为更明显是段落分隔
            const multiNewlineRegex = /\n\n+/;
            const hasMultiNewline = multiNewlineRegex.test(text);
            console.log('【分割器】规则2a检查：是否有多换行?', hasMultiNewline);
            
            if (hasMultiNewline) {
                // 按多换行分隔符分割
                const paragraphs = text.split(multiNewlineRegex);
                console.log('【分割器】多换行分割原始段落数:', paragraphs.length);
                
                // 过滤掉空段落，但保留有内容的段落
                const validParagraphs = paragraphs.filter(p => p.trim().length > 0);
                console.log('【分割器】过滤后有效段落数:', validParagraphs.length);
                
                if (validParagraphs.length > 1) {
                    console.log('【分割器】✓ 规则2a成功：按段落分隔符（多换行）分割:', validParagraphs.length, '段');
                    return validParagraphs;
                }
            }
            
            // **改进：规则2b - 检测是否有单个换行作为行分隔**
            // 如果有足够多的换行，这表示是多行内容，可能需要分割
            const singleNewlineCount = (text.match(/\n/g) || []).length;
            console.log('【分割器】规则2b检查：单换行数量=', singleNewlineCount);
            if (singleNewlineCount >= 3) {
                console.log('【分割器】规则2b：有足够的换行，尝试按行分组');
                
                // 有多行内容，尝试按换行分割
                const lines = text.split(/\n/);
                console.log('【分割器】行数:', lines.length);
                ...
                console.log('【分割器】行分组结果:', segments.length, '段');
                
                if (segments.length > 1) {
                    console.log('【分割器】✓ 规则2b成功：按行分组分割:', segments.length, '段');
                    return segments;
                }
            }
            
            // 规则3：找第一个合适的句号（中文标点或英文标点）
            // 优先选择靠近平均长度的分割点，避免分割太不均匀
            const avgLength = text.length / 2;
            ...
            for (const punct of punctuations) {
                let index = 0;
                while ((index = text.indexOf(punct, index)) !== -1) {
                    // 确保句号不在网址中（简单检查）
                    if (index > 0 && index < text.length - 1) {
                        const beforeChar = text[index - 1];
                        const afterChar = text[index + 1];
                        
                        // 避免在网址(.com)或其他非结句的地方分割
                        if (!(punct === '.' && /[a-zA-Z0-9]/.test(beforeChar) && /[a-zA-Z0-9]/.test(afterChar))) {
                            const distance = Math.abs(index - avgLength);
                            if (distance < bestDistance) {
                                bestDistance = distance;
                                bestSplitPoint = index + 1;
                            }
                        }
                    }
                    index++;
            ...
            if (bestSplitPoint > 0 && bestSplitPoint < text.length - 1) {
                return [
                    text.substring(0, bestSplitPoint),
                    text.substring(bestSplitPoint)
                ];
            }
            ...
            if (firstNewline > 0 && firstNewline < text.length - 1) {
                ...
                if (firstNewline > 50) {
                    return [
                        ...
                        text.substring(firstNewline).trim()
                    ];
                }
            }
            ...
            // 规则5：按固定长度分割（最后的退路）
            // 选择一个足够长的段落（避免过度碎片化）
            const maxSegmentLength = Math.min(300, Math.ceil(text.length / 2));
            if (text.length > maxSegmentLength * 1.5) {
                // 在最大长度处寻找最近的空格或标点作为分割点
                let splitPoint = maxSegmentLength;
                for (let i = maxSegmentLength; i < Math.min(maxSegmentLength + 50, text.length); i++) {
                    if (/[\s，。！？]/.test(text[i])) {
                        splitPoint = i + 1;
                        break;
                    }
        ...
        function parseThinkingProcess(text) {
            ...
            console.log('【parseThinking】开始检查');
            
            // **改进：检查是否有任何思考过程或多消息格式**
            // 1. 检查思考过程标签（THINK, THINKING等）
            // 2. 检查消息标签（REPLY, MSG）- 这也表示多消息格式
            
            const thinkRegex = /\[THINK\][\s\S]*?\[\/THINK\]|\[THINKING\][\s\S]*?\[\/THINKING\]|\[THOUGHT\][\s\S]*?\[\/THOUGHT\]|<think>[\s\S]*?<\/think>|<thinking>[\s\S]*?<\/thinking>|<thought>[\s\S]*?<\/thought>|\{\s*"thought"[\s\S]*?\}/gi;
            const hasThinking = thinkRegex.test(text);
            console.log('【parseThinking】有思考标签:', hasThinking);
            
            // **新增：直接检查是否有REPLY或MSG标签**
            const replyRegex = /\[REPLY\d+[\]:]|\[MSG\d+[\]:]/i;
            const hasReply = replyRegex.test(text);
            console.log('【parseThinking】有REPLY/MSG标签:', hasReply);
            
            if (!hasThinking && !hasReply) {
                console.log('【parseThinking】无思考过程也无多消息格式，返回null');
                return null;
            }
            ...
            console.log('【parseThinking】开始提取消息');
            
            // 重置正则表达式（因为之前做了test）
            const messageRegex = /\[REPLY(\d+)(?::([0-9.]+))?\]([\s\S]*?)\[\/REPLY\1\]|\[MSG(\d+)(?::([0-9.]+))?\]([\s\S]*?)\[\/MSG\4\]/gi;
            let match;
            
            while ((match = messageRegex.exec(text)) !== null) {
                const delay = (match[2] || match[5] || 0) * 1000;
                const content = match[3] || match[6] || '';
                
                console.log(`【parseThinking】找到消息，延迟:${delay}ms，内容:`, content.substring(0, 50));
            ...
            // 如果没有找到有延迟的消息，尝试查找简单的消息格式
            if (result.messages.length === 0) {
                console.log('【parseThinking】未找到有编号的REPLY，尝试简单格式');
                
                // 尝试从 [REPLY] 和 [/REPLY] 之间提取所有内容
                const simpleReplyRegex = /\[REPLY\]([\s\S]*?)\[\/REPLY\]/gi;
                while ((match = simpleReplyRegex.exec(text)) !== null) {
                    console.log('【parseThinking】找到简单REPLY:', match[1].substring(0, 50));
                    result.messages.push({
                        delay: 0,
                        content: match[1]
                    });
            ...
            // 如果仍然没有消息，返回null（表示虽然有思考过程，但没有实际消息要发送）
            if (result.messages.length === 0) {
                console.log('【parseThinking】仍无消息，尝试提取思考标签后的内容');
                
                // 尝试提取思考标签后的内容作为消息
                const afterThink = text.replace(/[\s\S]*?(?:\[\/THINK\]|<\/think>|<\/thinking>|\[\/THINKING\])/i, '').trim();
                if (afterThink) {
                    result.messages.push({
                        delay: 0,
                        content: afterThink
                    ...
                    console.log('【parseThinking】返回null，无有效内容');
                    return null;
                }
            }
            
            console.log('【parseThinking】成功返回结果，消息数:', result.messages.length);
            return result;
        }

        function appendAssistantMessage(convId, text) {
            console.log('【appendAssistantMessage】开始处理');
            
            // 首先检查是否包含思考过程格式
            const thinkingData = parseThinkingProcess(text);
            
            if (thinkingData) {
                console.log('【appendAssistantMessage】检测到思考过程，消息数:', thinkingData.messages.length);
                // 存在思考过程，分批添加消息
        ...
        function appendSingleAssistantMessage(convId, text) {
            console.log('【appendSingleAssistantMessage】开始处理');
            console.log('【调试】AI原始回复长度:', text.length);
            console.log('【调试】AI原始回复前300字:', text.substring(0, 300));
            
            // **现在 appendAssistantMessage 已经处理了思考过程检查**
            // 这个函数只需处理普通回复的分段
            
            // **第一步：尝试按段落分割**
            console.log('【调试】尝试按段落分割');
            const segments = goodEnoughSplitter(text);
            
            console.log('【调试】分割结果数量:', segments.length);
            if (segments.length > 1) {
                segments.forEach((seg, idx) => {
                    console.log(`【调试】段落${idx}(长度${seg.length}):`, seg.substring(0, 80));
                });
            }
            
            // 如果可以分割，分段推送
            if (segments.length > 1) {
                console.log(`【推送优化】将回复分为 ${segments.length} 段`);
                appendSegmentedMessages(convId, segments);
                return;
            }
            
            // **第二步：无法分割，按单条消息处理**
            console.log('【调试】未分割，使用单条消息模式');
            appendSingleSegmentMessage(convId, text);
        }
        
        // 分段推送消息
        function appendSegmentedMessages(convId, segments) {
            // 为每一段分配延迟，模拟自然的分段推送
            let currentDelay = 0;
            const segmentDelay = 800; // 每段推送间隔 800ms
            
            segments.forEach((segment, index) => {
                if (segment.trim()) {
                    const isLastSegment = (index === segments.length - 1);
                    // 最后一段不加延迟
                    const delay = isLastSegment ? 0 : segmentDelay;
                    
                    setTimeout(() => {
                        appendSingleSegmentMessage(convId, segment, isLastSegment);
                    }, currentDelay);
        ...
        // 处理单个文本段
        function appendSingleSegmentMessage(convId, text, isLastSegment = true) {
            console.log('【调试】处理消息段:', text.substring(0, 100), '是最后一段:', isLastSegment);
            
            // **重要：必须先提取心声，再清理内容！**
            // 仅在最后一段提取心声（前面的段没有心声标记）
            let mindState = null;
            ...
            // 2. 或文本末尾
            // 支持跨越多行和包含换行符的内容
            const mindStateRegex = /【心声】([\s\S]*?)(?=【|$)/;
            let match = text.match(mindStateRegex);
            
            console.log('【调试】正则匹配结果:', match ? '找到' : '未找到');
            
            // 仅在最后一段处理心声
            if (isLastSegment && match) {
                const mindContent = match[1].trim();
                mindState = {};
                
                console.log('【心声提取】原始内容:', mindContent);
                
                // 先提取好感度信息 - 改进正向前瞻，使其更灵活
                // 不要求后面必须有特定字段，允许字段间直接相邻
                const affectionRegex = /好感度[:：]\s*(\d+)\s*\|\s*([+-]\d+)\s*\|\s*([^]*?)(?=(?:穿搭|心情|动作|心声|坏心思)[:：]|$)/;
                const affectionMatch = mindContent.match(affectionRegex);
                
                if (affectionMatch) {
                    mindState.affection = {
                        value: parseInt(affectionMatch[1]),
                        change: parseInt(affectionMatch[2]),
                        reason: affectionMatch[3].trim()
                    };
                    console.log('【好感度提取】成功:', mindState.affection);
                } else {
                    console.log('【好感度提取】失败，测试内容:', mindContent.substring(0, 100));
                    // 如果找不到标准格式，尝试更宽松的匹配
                    const looseAffection = mindContent.match(/(\d+)\s*\|\s*([+-]\d+)\s*\|/);
                    if (looseAffection) {
                        mindState.affection = {
                            value: parseInt(looseAffection[1]),
                            change: parseInt(looseAffection[2]),
                            reason: '情感变化'
                        };
                        console.log('【好感度提取】宽松匹配:', mindState.affection);
                    }
                }
                ...
                for (const [key, label] of Object.entries(fields)) {
                    // 改进：不要求前面有空格，字段可以直接相邻
                    // 从字段标签开始，匹配到下一个字段标签（:或：）或内容结尾
                    const fieldRegex = new RegExp(`${label}[:：]\\s*([^]*?)(?=(?:穿搭|心情|动作|心声|坏心思)[:：]|$)`);
                    const fieldMatch = mindContent.match(fieldRegex);
                    if (fieldMatch && fieldMatch[1]) {
                        let fieldValue = fieldMatch[1].trim();
                        // 改进：使用非贪婪匹配，只清理实际的字段标签，不用.*$这样的贪婪模式
                        fieldValue = fieldValue.replace(/\s*(?:穿搭|心情|动作|心声|坏心思)[:：].*$/s, '').trim();
                        if (fieldValue) {
                            mindState[key] = fieldValue;
                            console.log(`【${label}提取】成功:`, fieldValue.substring(0, 30));
                        }
                    }
                }
                ...
                if (Object.keys(mindState).length > 0) {
                    console.log('【心声更新】保存心声数据:', mindState);
                    updateCharacterMindState(convId, mindState);
                } else {
                    console.log('【心声更新】未能解析心声内容:', mindContent);
                }
            } else if (!isLastSegment) {
                console.log('【心声提取】跳过（这不是最后一段）');
            }
            
            // 从显示的文本中完全移除心声部分
            text = text.replace(/【心声】[\s\S]*?(?=【|$)/g, '').trim();
            
            // **现在才调用cleanAIResponse（心声已经被安全提取和移除了）**
            ...
            const aiMsg = {
                ...
                time: new Date().toISOString(),
                apiCallRound: apiCallRounds[convId]  // 使用当前会话的API调用回合标记
            };
        ...
        function appendMultipleAssistantMessages(convId, thinkingData) {
            ...
            console.log('【appendMultiple】开始处理多条消息，数量:', messages.length);
            messages.forEach((msg, idx) => {
                console.log(`  消息${idx}: 延迟${msg.delay}ms, 内容:`, msg.content.substring(0, 50));
            });
            
            // 最重要：从原始的完整文本中提取所有【心声】标记
            // 支持多条消息中的心声可能在不同位置的情况
            let globalMindState = null;
            const originalText = thinkingData.originalText || '';
            
            // 修复：使用 /g 全局标志，处理所有【心声】出现，不只是第一个
            // 支持跨越多行和包含换行符的内容
            const globalMindStateRegex = /【心声】([\s\S]*?)(?=【|$)/g;
            let allMindStates = [];
            let globalMatch;
            
            while ((globalMatch = globalMindStateRegex.exec(originalText)) !== null) {
                allMindStates.push(globalMatch[1].trim());
            }
            
            // 合并所有心声数据（取最后一条的详细信息）
            let currentMindStateContent = allMindStates.length > 0 ? allMindStates[allMindStates.length - 1] : '';
            
            if (currentMindStateContent) {
                const mindContent = currentMindStateContent;
                globalMindState = {};
                
                console.log('【全局心声提取】原始内容:', mindContent);
                
                // 先提取好感度信息 - 改进正向前瞻，不要求前面有空格
                const affectionRegex = /好感度[:：]\s*(\d+)\s*\|\s*([+-]\d+)\s*\|\s*([^]*?)(?=(?:穿搭|心情|动作|心声|坏心思)[:：]|$)/;
                const affectionMatch = mindContent.match(affectionRegex);
                
                if (affectionMatch) {
                    globalMindState.affection = {
                        value: parseInt(affectionMatch[1]),
                        change: parseInt(affectionMatch[2]),
                        reason: affectionMatch[3].trim()
                    };
                    console.log('【全局好感度提取】成功:', globalMindState.affection);
                } else {
                    const looseAffection = mindContent.match(/(\d+)\s*\|\s*([+-]\d+)\s*\|/);
                    if (looseAffection) {
                        globalMindState.affection = {
                        ...
                        console.log('【全局好感度提取】宽松匹配:', globalMindState.affection);
                    }
                }
                ...
                for (const [key, label] of Object.entries(fields)) {
                    ...
                    if (fieldMatch && fieldMatch[1]) {
                        let fieldValue = fieldMatch[1].trim();
                        // 改进：只清理实际的字段标签，不用贪婪模式
                        fieldValue = fieldValue.replace(/\s*(?:穿搭|心情|动作|心声|坏心思)[:：].*$/s, '').trim();
                        if (fieldValue) {
                            globalMindState[key] = fieldValue;
                            console.log(`【全局${label}提取】成功:`, fieldValue.substring(0, 30));
                        }
                    }
                }
                
                if (Object.keys(globalMindState).length > 0) {
                    console.log('【全局心声更新】保存心声数据:', globalMindState);
                    updateCharacterMindState(convId, globalMindState);
                } else {
                    console.log('【全局心声更新】未能解析心声内容:', mindContent);
                }
            } else {
                console.log('【全局心声提取】未找到心声标记在完整响应中');
            }
            
            // 记录所有找到的心声（便于调试）
            if (allMindStates.length > 1) {
                console.log(`【多条心声检测】找到 ${allMindStates.length} 条心声`);
            }
            
            messages.forEach((msgData, index) => {
                console.log(`【appendMultiple】消息${index}将在${currentDelay}ms后推送`);
                
                setTimeout(() => {
                    console.log(`【appendMultiple】消息${index}现在推送 @${Date.now()}`);
                    ...
                    // 从内容中移除心声部分（避免在消息显示中显示）
                    content = content.replace(/【心声】[^\n]*\n?/g, '').trim();
                    content = content.replace(/【心声】[\s\S]*$/g, '').trim();
        ...
        function openCharacterMindState(chat) {
            ...
            // 初始化好感度历史
            if (!chat.affectionHistory) {
                chat.affectionHistory = [];
            }
            ...
            let affectionReason = '';
            if (chat.affectionHistory && chat.affectionHistory.length > 0) {
                const latest = chat.affectionHistory[chat.affectionHistory.length - 1];
                currentAffection = latest.value;
                affectionChange = latest.change;
                affectionReason = latest.reason;
            }
            
            // 计算好感度颜色
            let affectionColor = '#999';
            if (currentAffection >= 75) {
                affectionColor = '#ff6b9d'; // 粉红色，高好感度
            } else if (currentAffection >= 50) {
                affectionColor = '#ffa500'; // 橙色，中等好感度
            } else if (currentAffection >= 25) {
                affectionColor = '#ffcc00'; // 黄色，低好感度
            } else {
                affectionColor = '#999'; // 灰色，极低好感度
            }
            ...
            let content = `
                <div class="emoji-mgmt-content" style="max-width:400px;background:#f5f5f5;display:flex;flex-direction:column;max-height:80vh;">
                    ...
                    <div style="padding:16px;background:#fff;margin-bottom:0;flex:1;overflow-y:auto;overflow-x:hidden;">
                        <!-- 好感度显示 -->
                        <div style="margin-bottom:16px;padding:12px;background:linear-gradient(135deg, ${affectionColor}20, ${affectionColor}10);border-radius:4px;border-left:4px solid ${affectionColor};">
                            <div style="font-size:14px;color:#333;font-weight:600;margin-bottom:6px;">好感度</div>
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                                <div style="font-size:24px;color:${affectionColor};font-weight:bold;">${currentAffection}</div>
                                <div style="font-size:13px;color:${affectionChange > 0 ? '#4CAF50' : (affectionChange < 0 ? '#f44' : '#999')};">${affectionChange > 0 ? '+' : ''}${affectionChange}</div>
                            </div>
                            <div style="width:100%;height:6px;background:#f0f0f0;border-radius:3px;overflow:hidden;margin-bottom:6px;">
                                <div style="width:${currentAffection}%;height:100%;background:${affectionColor};"></div>
                            </div>
                            <div style="font-size:12px;color:#666;">${affectionReason || '心情平稳'}</div>
                        </div>
            `;
        ...
        function showCharacterMindHistory(charId) {
            ...
            if (chat.mindStates && chat.mindStates.length > 0) {
                ...
                for (let i = chat.mindStates.length - 1; i >= 0; i--) {
                    ...
                    // 获取该记录的好感度信息（从affectionHistory中查找）
                    let affectionDisplay = '';
                    if (chat.affectionHistory && chat.affectionHistory.length > 0) {
                        const affectionIndex = Math.min(i, chat.affectionHistory.length - 1);
                        if (affectionIndex >= 0) {
                            const aff = chat.affectionHistory[affectionIndex];
                            let affectionColor = '#999';
                            if (aff.value >= 75) affectionColor = '#ff6b9d';
                            else if (aff.value >= 50) affectionColor = '#ffa500';
                            else if (aff.value >= 25) affectionColor = '#ffcc00';
                            
                            affectionDisplay = `<div style="margin-bottom:8px;padding:8px;background:${affectionColor}20;border-left:3px solid ${affectionColor};border-radius:2px;">
                                <div style="font-size:12px;color:#666;">好感度：<span style="color:${affectionColor};font-weight:bold;">${aff.value}</span> <span style="color:${aff.change > 0 ? '#4CAF50' : (aff.change < 0 ? '#f44' : '#999')};">${aff.change > 0 ? '+' : ''}${aff.change}</span></div>
                                <div style="font-size:11px;color:#999;">${aff.reason}</div>
                            </div>`;
                        }
                    }
                    ...
                    historyContent += `
                        <div style="margin-bottom:16px;padding:12px;background:#f9f9f9;border-radius:4px;border-left:3px solid #333;position:relative;">
                            ...
                            ${affectionDisplay}
                            ${Object.entries(state).filter(([key]) => key !== 'affection').map(([key, value]) => {
                                const labels = {
                                    'outfit': '穿搭',
                                    'mood': '心情',
                                    'action': '动作',
        ...
        function updateCharacterMindState(charId, mindData) {
            ...
            // 如果心声中包含好感度信息，保存到好感度历史
            if (mindData.affection) {
                chat.affectionHistory.push({
                    value: mindData.affection.value,
                    change: mindData.affection.change,
                    reason: mindData.affection.reason,
                    timestamp: new Date().toISOString()
                });
            }
