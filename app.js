        ...
        const AppState = {
            ...
            apiSettings: {
                ...
                // 副API功能提示词
                secondaryPrompts: {
                    mindState: '根据以下对话内容，深入分析角色的当前心理状态和真实想法。请严格按照以下格式返回，每个字段单独成行，用中文冒号分隔，不能省略任何字段，每个字段都必须有明确内容：\n\n【心声】\n穿搭：具体描述角色的衣着、配饰、整体风格（不少于50字）\n心情：细腻描述角色的情绪状态，可包含矛盾（不少于50字）\n动作：描述角色正在做或习惯的小动作（不少于30字）\n心声：角色内心真实想法，可包含犹豫、期待等（不少于50字）\n坏心思：角色的真实想法或小计划，必须填写有意义内容（不少于30字）\n好感度：用数字表示，0-100之间的整数\n好感度变化：变化数值，用整数表示，如+5、-2或0\n好感度原因：简短说明，10字以内\n\n重要规则：\n1. 格式必须严格遵循，每行一个字段\n2. 每个字段的冒号后面紧跟内容，不能为空\n3. 不要加任何其他说明或额外文字\n4. 所有字段都必须填写',
    ...
    thirdParty: true      // 第三方
        ...
        function initEventListeners() {
            ...
            window.handleDoubleClickAvatar = async function() {
                ...
                if (apiResult && typeof apiResult.then === 'function') {
                    apiResult.then(() => {
                        console.log('========== 🎯 主API回复完成，准备调用副API生成心声 ==========');
                        console.log('当前对话:', AppState.currentChat ? AppState.currentChat.id : '无');
                        // 延迟200ms后触发心声生成，确保AI回复已完成
                        setTimeout(() => {
                            console.log('========== ⏱️ 延迟时间已到，正在调用副API ==========');
                            if (AppState.currentChat) {
                                generateCharacterMindStateViaSecondaryAPI(AppState.currentChat.id);
                            } else {
                                console.error('❌ 无法调用副API：AppState.currentChat为空');
                            }
                        }, 200);
                    }).catch(err => {
                        console.error('❌ 主API错误，跳过心声生成:', err);
                    });
                } else {
                    console.warn('⚠️ apiResult不是Promise，无法调用副API');
                }
        ...
        // 生成心声 - 由副API处理
        // 为副API收集消息（不包含系统提示词，只要求核心对话内容）
        function collectConversationForSecondaryAPI(convId) {
            ...
            messagesToSend.forEach(m => {
                ...
                if (m.isImage) {
                    if (m.photoDescription) {
                        messageContent = `[图片，描述：${m.photoDescription}]`;
                    } else {
                        messageContent = '[图片]';
                    }
                }
            ...
            // 修复：如果最后一条消息来自AI角色，需要添加一条占位符用户消息
            // 这样可以确保心声生成时，AI能够基于完整的对话上下文进行分析
            if (out.length > 0 && out[out.length - 1].role === 'assistant') {
                console.log('⚠️ 检测到最后一条消息来自AI，添加占位符用户消息以完成对话');
        ...
        function generateCharacterMindStateViaSecondaryAPI(convId, retryCount = 0) {
            const conv = AppState.conversations.find(c => c.id === convId);
            if (!conv) {
                console.error('❌ 对话未找到，convId:', convId);
                showToast('对话未找到');
                return;
            }
            ...
            if (retryCount > maxRetries) {
                console.error('❌ 心声生成重试次数过多，已放弃');
                showToast('心声生成失败：已达到重试次数限制，请检查API返回格式');
                return;
            }

            // 使用专门的副API消息收集函数
            const messages = collectConversationForSecondaryAPI(convId);
            
            console.log('🔍 副API消息收集结果:', {
                convId,
                messageCount: messages.length,
                retryCount: retryCount,
                messages: messages.map(m => ({ role: m.role, contentLength: m.content.length, preview: m.content.substring(0, 50) }))
            });

            if (messages.length === 0) {
                console.warn('⚠️ 对话消息为空，无法生成心声');
                showToast('对话消息为空，无法生成心声');
                return;
            }

            // 使用动态提示词 - 优化格式要求
            const systemPrompt = AppState.apiSettings?.secondaryPrompts?.mindState || 
                '根据以下对话内容，深入分析角色的当前心理状态和真实想法。请严格按照以下格式返回，每个字段占一行，不能省略任何字段：\n\n【心声】\n穿搭：{具体描述}\n心情：{细腻描述}\n动作：{具体描述}\n心声：{真实想法}\n坏心思：{隐形念头}\n好感度：{0-100的数字}\n好感度变化：{整数}\n好感度原因：{简短说明}';

            console.log('📝 副API提示词:', systemPrompt.substring(0, 100) + '...');
            showToast('正在生成心声...');
            ...
            callSecondaryAPI(
                ...
                systemPrompt,
                (result) => {
                    console.log('✅ 副API返回结果:', result);
                    
                    // 查找【心声】标记
                    const mindMarkerIndex = result.indexOf('【心声】');
                    
                    console.log('🔎 搜索【心声】标记:', {
                        found: mindMarkerIndex !== -1,
                        position: mindMarkerIndex,
                        resultPreview: result.substring(0, 200)
                    });
                    
                    if (mindMarkerIndex === -1) {
                        console.error('❌ 未找到【心声】标记', result);
                        if (retryCount < maxRetries) {
                            console.warn(`⚠️ 第${retryCount + 1}次重试...`);
                            showToast('API返回格式不符，正在重新生成...');
                            setTimeout(() => {
                                generateCharacterMindStateViaSecondaryAPI(convId, retryCount + 1);
                            }, 1500);
                        } else {
                            showToast('心声生成失败：API返回格式不符（缺少【心声】标记）');
                        }
                        return;
                    }
                    
                    // 提取【心声】之后的所有内容
                    const mindContent = result.substring(mindMarkerIndex + 5).trim();
                    console.log('📋 心声内容:', mindContent);
                    ...
                    console.log('📑 分割成行数:', lines.length);
                    ...
                    for (const line of lines) {
                        ...
                        for (const [key, label] of Object.entries(fields)) {
                            // 跳过已经找到的字段
                            if (mindState[key]) continue;
                            
                            // 构建更灵活的正则匹配模式
                            // 支持 "标签:" "标签：" "标签: " "标签： " 等多种格式
                            const escapedLabel = label.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                            const patterns = [
                                `^\\s*${escapedLabel}[：:]{1,2}\\s*(.+)$`,  // 行开头
                                `^\\s*${escapedLabel}[：:]{1,2}\\s*(.+?)(?=\\s*(?:${Object.values(fields).map(l => l.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})[：:]|$)`,  // 行中间
                            ];
                            ...
                            for (const pattern of patterns) {
                                ...
                                if (match && match[1]) {
                                    ...
                                    console.log(`✓ 匹配到${key}(${label}):`, foundValue.substring(0, 50));
                                    break;
                                }
                            }
                            ...
                            if (foundValue) {
                                // 移除可能的多余标记或其他字段标签
                                foundValue = foundValue.replace(/【.*?】/g, '').trim();
                                // 清理末尾可能的其他字段标签
                                const fieldLabels = Object.values(fields).join('|');
                                foundValue = foundValue.replace(new RegExp(`(${fieldLabels})[：:]*$`), '').trim();
                                
                                // 特殊处理数值字段
                                if (key === 'affinity') {
                                    const affinityNum = parseInt(foundValue);
                                    if (!isNaN(affinityNum)) {
                                        mindState[key] = Math.max(0, Math.min(100, affinityNum));
                                        console.log(`✓ 好感度数值: ${mindState[key]}`);
                                    }
                                } else if (key === 'affinityChange') {
                                    const changeNum = parseInt(foundValue);
                                    if (!isNaN(changeNum)) {
                                        mindState[key] = changeNum;
                                        console.log(`✓ 好感度变化数值: ${mindState[key]}`);
                                    }
                                } else if (key === 'affinityReason') {
                                    mindState[key] = foundValue ? foundValue.substring(0, 10) : null;
                                } else {
                                    // 其他字段保存值
                                    if (foundValue) {
                                        mindState[key] = foundValue;
                                    }
                                }
                            }
                        }
                    ...
                    // 调试输出
                    console.log('🧠 最终心声解析结果:', mindState);
                    console.log('📊 解析统计:', {
                        fieldsExtracted: Object.keys(mindState).length,
                        extractedFields: Object.keys(mindState),
                        emptyFields: Object.keys(mindState).filter(k => !mindState[k])
                    });

                    
                    // 检查必要字段是否都被提取且都有有效值
                    const requiredFields = ['affinity', 'affinityChange', 'affinityReason', 'outfit', 'mood', 'action', 'thought', 'badThought'];
                    const extractedFields = Object.keys(mindState);
                    const missingFields = requiredFields.filter(f => !extractedFields.includes(f) || mindState[f] === null || mindState[f] === '');
                    ...
                    if (extractedFields.length > 0) {
                        if (missingFields.length > 0) {
                            console.warn(`❌ 缺少字段或字段为空，第${retryCount + 1}次重试:`, missingFields);
                            
                            if (retryCount < maxRetries) {
                                // 缺少字段时，重新调用API获取完整心声
                                showToast('心声信息不完整，正在重新生成...');
                                
                                // 延迟后重新调用生成心声
                                setTimeout(() => {
                                    console.log(`⏱️ 开始第${retryCount + 2}次重试...`);
                                    generateCharacterMindStateViaSecondaryAPI(convId, retryCount + 1);
                                }, 1500);
                                return;
                            } else {
                                showToast('心声信息仍不完整，已达重试限制。缺少字段: ' + missingFields.join(', '));
                                return;
                            }
                        }
                        console.log('✅ 心声所有字段完整，保存到存储...');
                        updateCharacterMindState(convId, mindState);
                        saveToStorage();
                        showToast('🎉 心声已生成');
                    } else {
                        console.error('❌ 无法解析任何字段，API返回可能有问题');
                        if (retryCount < maxRetries) {
                            showToast('无法解析字段，正在重新生成...');
                            setTimeout(() => {
                                generateCharacterMindStateViaSecondaryAPI(convId, retryCount + 1);
                            }, 1500);
                        } else {
                            showToast('心声生成失败：无法解析心声信息，已达重试限制');
                        }
                    }
                },
                (error) => {
                    console.error('❌ 副API调用失败:', error);
                    showToast('副API调用失败：' + (error ? error.toString() : '未知错误'));
                }
            );
        }
        ...
        function translateMessageViaSecondaryAPI(msgId, targetLanguage = '英文') {
            ...
            const content = msg.content;
            const systemPrompt = targetLanguage === '英文' 
                ? '你是一个翻译助手。将用户提供的中文文本翻译成英文。只返回翻译结果，不要有其他内容。'
                : '你是一个翻译助手。将用户提供的文本翻译成中文。只返回翻译结果，不要有其他内容。';
        ...
        // 手动总结 - 使用副API
        function summarizeConversationViaSecondaryAPI(convId, isAutomatic = false) {
            const conv = AppState.conversations.find(c => c.id === convId);
            if (!conv) {
                showToast('对话未找到');
                return;
            ...
            // 使用动态提示词
            const systemPrompt = AppState.apiSettings?.secondaryPrompts?.summarize || 
                '你是一个专业的对话总结员。请为对话内容生成一份简洁准确的总结。总结应该抓住核心内容、保留重要信息、简洁明了（200-300字）。';
        ...
        function translateToChineseViaAPI(text, msg) {
            // 使用副API翻译
            const systemPrompt = AppState.apiSettings?.secondaryPrompts?.translateChinese || 
                '你是一个翻译助手。将用户提供的非中文文本翻译成简体中文。只返回翻译结果，不要有其他内容。';
        ...
        function translateToEnglishViaAPI(text, msg) {
            // 使用副API翻译
            const systemPrompt = AppState.apiSettings?.secondaryPrompts?.translateEnglish || 
                '你是一个翻译助手。将用户提供的中文文本翻译成英文。只返回翻译结果，不要有其他内容。';
...
6. 可以分多条消息回复（重要）`);
            ...
            // 修复：如果最后一条消息来自AI角色，需要添加一条占位符用户消息
            // 这样可以确保主API回复时，能够基于完整的对话上下文进行分析
            // 查找最后一条非system消息
            ...
            if (lastNonSystemIndex !== -1 && out[lastNonSystemIndex].role === 'assistant') {
                console.log('⚠️ 检测到最后一条消息来自AI，添加占位符用户消息以完成对话');
                out.push({ role: 'user', content: '...' });
            }
        ...
        function openCharacterMindState(chat) {
            ...
            mindItems.forEach(item => {
                ...
                content += `
                    <div style="margin-bottom:12px;padding:12px;background:#f9f9f9;border-radius:4px;border-left:3px solid #333;">
                        <div style="font-size:14px;color:#333;font-weight:600;margin-bottom:4px;">${item.label}</div>
                        <div style="font-size:13px;color:${displayValue === null ? '#ccc' : '#666'};word-break:break-all;">${displayValue === null ? '尚未生成' : escapeHtml(String(displayValue))}</div>
                    </div>
                `;
            });
