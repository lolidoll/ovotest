    ...
    thirdParty: true      // 第三方
        ...
        function generateCharacterMindStateViaSecondaryAPI(convId) {
            const conv = AppState.conversations.find(c => c.id === convId);
            if (!conv) {
            ...
            const systemPrompt = AppState.apiSettings?.secondaryPrompts?.mindState || 
                '根据以下对话内容，深入分析角色的当前心理状态和真实想法。请严格按照以下格式返回，不能省略任何字段，不能为空：\n\n【心声】\n穿搭：{具体描述}\n心情：{细腻描述}\n动作：{具体描述}\n心声：{真实想法}\n坏心思：{隐形念头}\n好感度：{0-100数字}\n好感度变化：{整数}\n好感度原因：{简短说明}';
                ...
                (result) => {
                    ...
                    for (const line of lines) {
                        ...
                        for (const [key, label] of Object.entries(fields)) {
                            // 创建灵活的匹配模式，支持冒号和中文冒号
                            const patterns = [
                                `^\\s*${label}[：:]{1,2}\\s*(.+?)(?:\\s*[${Object.values(fields).join('')}]|$)`,
                                `${label}[：:]{1,2}\\s*(.+?)(?:\\s*(?:${Object.values(fields).join('|')})[：:]|$)`,
                                `${label}[：:]{1,2}\\s*(.+)`
                            ];
                            ...
                            for (const pattern of patterns) {
                                ...
                                if (match && match[1]) {
                                    ...
                                    console.log(`✓ 匹配到${key}(${label}):`, foundValue);
                                    break;
                                }
                            }
                    ...
                    console.log('📊 解析统计:', {
                        ...
                        emptyFields: Object.keys(mindState).filter(k => !mindState[k]).length
                    });
                    ...
                    if (extractedFields.length > 0) {
                        if (missingFields.length > 0) {
                            console.warn('❌ 缺少字段或字段为空，需要重新生成:', missingFields);
                            // 缺少字段时，重新调用API获取完整心声
                            showToast('心声信息不完整，正在重新生成完整内容...');
                            
                            // 延迟后重新调用生成心声
                            setTimeout(() => {
                                console.log('⏱️ 开始重新调用副API...');
                        ...
                        showToast('心声生成失败：无法解析心声信息（API返回格式可能有误）');
                    }
                },
                (error) => {
