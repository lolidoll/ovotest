        ...
        const AppState = {
            ...
            apiSettings: {
                ...
                // 副API功能提示词
                secondaryPrompts: {
                    mindState: '你是一个角色分析师。根据对话内容，分析角色的当前状态和想法。\n请按以下格式返回心声信息：\n【心声】\n穿搭：{当前穿着打扮描述}\n心情：{当前心情状态}\n动作：{当前在做的动作或姿态}\n心声：{内心真实想法}\n坏心思：{隐藏的不良想法，如无则留空}',
    ...
    thirdParty: true      // 第三方
        ...
        function generateCharacterMindStateViaSecondaryAPI(convId) {
            ...
            const systemPrompt = AppState.apiSettings?.secondaryPrompts?.mindState || 
                '你是一个角色分析师。根据对话内容，分析角色的当前状态和想法。请按格式返回心声信息。';
            ...
            callSecondaryAPI(
                ...
                (result) => {
                    ...
                    const match = result.match(mindStateRegex);
                    ...
                    if (match) {
                        ...
                        if (Object.keys(mindState).length > 0) {
                            ...
                            showToast('心声生成失败：无法解析心声信息');
                        }
                    } else {
                        showToast('心声生成失败：响应格式错误');
                    }
                },
                (error) => {
                    console.error('生成心声出错:', error);
                }
            );
        }
