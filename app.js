...
6. 可以分多条消息回复（重要）`);
        ...
        function appendSingleAssistantMessage(convId, text) {
            ...
            // 匹配整行心声信息：【心声】开头到换行符或文末
            // 更灵活的正则 - 匹配【心声】后的所有内容，直到行尾或下一个特殊标记
            const mindStateRegex = /【心声】[^\n]*(?:\n|$)/;
            let match = text.match(mindStateRegex);
            
            // 如果没有匹配到，尝试匹配从【心声】到文末的所有内容
            if (!match) {
                const altRegex = /【心声】[\s\S]*?(?=\n\n|$)/;
            ...
            if (match) {
                ...
                // 先提取好感度信息 - 更灵活的正则
                const affectionRegex = /好感度[:：]\s*(\d+)\s*\|\s*([+-]\d+)\s*\|\s*([^]*?)(?=\s+(?:穿搭|心情|动作|心声|坏心思)|$)/;
                ...
                for (const [key, label] of Object.entries(fields)) {
                    // 灵活的字段匹配：从字段名开始，匹配到下一个字段名或行尾
                    const fieldRegex = new RegExp(`${label}[:：]\\s*([^]*?)(?=\\s+(?:穿搭|心情|动作|心声|坏心思)|$)`);
        ...
        function appendMultipleAssistantMessages(convId, thinkingData) {
            ...
            const globalMindStateRegex = /【心声】[^\n]*(?:\n|$)/;
            let globalMatch = originalText.match(globalMindStateRegex);
            
            if (!globalMatch) {
                const altRegex = /【心声】[\s\S]*?(?=\n\n|$)/;
                globalMatch = originalText.match(altRegex);
            }
            
            if (globalMatch) {
                const mindText = globalMatch[0];
                const mindContent = mindText.replace(/【心声】/, '').trim();
                globalMindState = {};
