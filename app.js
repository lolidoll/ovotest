...
6. 可以分多条消息回复（重要）`);
        ...
        function cleanAIResponse(text) {
            ...
            // XML/HTML格式的思维链标签（优先处理，因为这是最常见的格式）
        ...
        function parseThinkingProcess(text) {
            ...
            // 支持多种思考过程格式
            const thinkRegex = /\[THINK\][\s\S]*?\[\/THINK\]|\[THINKING\][\s\S]*?\[\/THINKING\]|<think>[\s\S]*?<\/think>|<thinking>[\s\S]*?<\/thinking>/gi;
        ...
        function appendMultipleAssistantMessages(convId, thinkingData) {
            ...
            // 最重要：从原始的完整文本中提取心声
            // 因为心声可能在 [THINK]...[/THINK] 之外
            let globalMindState = null;
            const originalText = thinkingData.originalText || '';
            
            // 改进：从【心声】开始，匹配所有内容直到【...】或文末
            // 支持跨越多行和包含换行符的内容
            const globalMindStateRegex = /【心声】([\s\S]*?)(?=【|$)/;
            let globalMatch = originalText.match(globalMindStateRegex);
            
            if (globalMatch) {
                const mindContent = globalMatch[1].trim();
                globalMindState = {};
